...
class BetterFormattedText:
    def __init__(self, plain_text):
        self.plain_text = plain_text
        '''
        ech formatting can start and end on some position and can be deferent formation one from another
        so the implementation is to create inner class that handle formating position and style 
        '''
        self.formatting = []
    ...
    def __str__(self):
        ...
        for i in range(len(self.plain_text)):
            ...
            for f in self.formatting:
                if f.covers(i) and f.capitalize:
                    c = c.upper()
            result.append(c)
        return ''.join(result)
...
if __name__ == '__main__':
    ...
    bft.set_range(16, 19).capitalize = True
    print(bft)